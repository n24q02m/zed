...
enum StashKind {
    ...
    Tracked,
    Staged,
    Selected(RepositoryId, Vec<RepoPath>),
}
...
/// repository-scoped change key, directories select as a unit covering their
/// descendants.
enum RowMark {
    File(ChangeKey),
    Directory(TreeKey),
...
impl MarkedDirectoryCoverage {
    ...
    fn observe(&mut self, entry: &GitListEntry, marked_directories: &HashSet<TreeKey>) -> bool {
        let depth = match entry {
            // Repository and section boundaries always end directory coverage:
            // a marked directory can only cover rows inside its own repository
            // and section.
            GitListEntry::RepositoryHeader(_)
            | GitListEntry::ProjectRepositoriesHeader(_)
            | GitListEntry::Header(_) => {
...
pub struct GitPanel {
    ...
    collapsed_sections_by_repository: HashSet<(RepositoryId, Section)>,
    ...
    marked_paths: HashSet<ChangeKey>,
...
impl GitPanel {
    ...
    fn row_mark(&self, ix: usize) -> Option<RowMark> {
        match self.entries.get(ix)? {
            GitListEntry::Status(entry) => Some(RowMark::File(ChangeKey {
                repository_id: self.repository_id_for_entry_index(ix)?,
                repo_path: entry.repo_path.clone(),
            })),
            GitListEntry::TreeStatus(entry) => Some(RowMark::File(ChangeKey {
                repository_id: self.repository_id_for_entry_index(ix)?,
                repo_path: entry.entry.repo_path.clone(),
            })),
    ...
    fn mark_row(&mut self, ix: usize) {
        match self.row_mark(ix) {
            Some(RowMark::File(key)) => {
                self.marked_paths.insert(key);
            }
    ...
    fn toggle_mark(&mut self, ix: usize, cx: &mut Context<Self>) {
        ...
        if let Some(mark) = self.row_mark(ix) {
            ...
            match mark {
                RowMark::File(key) => {
                    if !self.marked_paths.remove(&key) {
                        self.marked_paths.insert(key);
                    }
                }
    ...
    /// directly plus the descendants of marked directories, each tagged with
    /// its owning repository so bulk actions can fan out per repository.
    fn marked_file_entries(&self) -> Vec<(RepositoryId, GitStatusEntry)> {
        ...
        for (ix, entry) in self.entries.iter().enumerate() {
            ...
            if let Some(status_entry) = entry.status_entry() {
                let Some(repository_id) = self.repository_id_for_entry_index(ix) else {
                    continue;
                };
                let key = ChangeKey {
                    repository_id,
                ...
                if covered || self.marked_paths.contains(&key) {
                    marked.push((repository_id, status_entry.clone()));
                }
            }
        }
        marked
    ...
    fn effective_status_entries(&self) -> Vec<(RepositoryId, GitStatusEntry)> {
        let selected = self.selected_entry.and_then(|ix| {
            let entry = self.entries.get(ix)?.status_entry()?;
            let repository_id = self.repository_id_for_entry_index(ix)?;
            Some((ix, repository_id, entry))
        });
        if let Some((selected_ix, repository_id, selected)) = selected {
            let use_selection = match self.marked_paths.len() + self.marked_directories.len() {
                0 => true,
                1 => {
                    !(self.marked_directories.is_empty()
                        && self.marked_paths.contains(&ChangeKey {
                            repository_id,
            ...
            if use_selection {
                return vec![(repository_id, selected.clone())];
            }
            let _ = selected_ix;
        }
    ...
    fn entry_identity(&self, index: usize) -> Option<GitPanelEntryId> {
        ...
        let repo_id = self
            .repository_id_for_entry_index(index)
            .or_else(|| self.active_repository_id)?;
    ...
    fn visible_selectable_entry_ids(&self) -> Vec<GitPanelEntryId> {
        // `visible_entry_indices` is already the authoritative visible list in
        // both view modes (built with repository/section/directory collapse
        // applied), so it is used directly here.
    ...
    fn select_first(
        ...
        if let Some(first_entry) = first_entry {
            ...
            self.selected_entry_id = self.entry_identity(first_entry);
            self.selection_anchor = self.selected_entry_id.clone();
            self.scroll_to_selected_entry(cx);
        }
    }
    ...
    fn select_previous(
        ...
        if window.modifiers().shift {
            self.apply_range_gesture(selected_entry, candidate);
        } else {
            self.mark_range_gesture = None;
        };

        self.selected_entry = Some(candidate);
        self.selected_entry_id = self.entry_identity(candidate);
        self.selection_anchor = self.selected_entry_id.clone();
        self.scroll_to_selected_entry(cx);
    }
    ...
    fn select_last(&mut self, _: &menu::SelectLast, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Some(last_entry) = last_entry {
            ...
            self.selected_entry_id = self.entry_identity(last_entry);
            self.selection_anchor = self.selected_entry_id.clone();
            self.scroll_to_selected_entry(cx);
        }
    }
    ...
    fn revert_entries(
        &mut self,
        entries: Vec<(RepositoryId, GitStatusEntry)>,
        skip_prompt: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) {
        ...
        let prompt = if skip_prompt {
            ...
            let mut details = entries
                .iter()
                .filter_map(|(_, entry)| entry.repo_path.as_ref().file_name())
            ...
            let all_created = entries.iter().all(|(_, entry)| entry.status.is_created());
        ...
        window
            .spawn(cx, async move |cx| {
                ...
                let created = this.update_in(cx, |this, window, cx| {
                    let staged = entries
                        .iter()
                        .filter(|(_, entry)| entry.status.staging().has_staged())
                        .cloned()
                        .collect::<Vec<_>>();
                    if !staged.is_empty() {
                    ...
                    for (repository_id, entry) in entries
                        ...
                        .cloned()
                    {
                        tracked_by_repository
                            .entry(repository_id)
                            .or_default()
                            .push(entry);
                    }
                    for (repository_id, tracked) in tracked_by_repository {
                ...
                let tasks = workspace.update(cx, |workspace, cx| {
                    created
                        .iter()
                        .filter_map(|(repository_id, entry)| {
    ...
    /// Fans a repo-tagged entry set out to one stage/unstage call per owning
    /// repository. Entries whose repository is no longer live are skipped.
    fn change_file_stage_grouped(
        &mut self,
        stage: bool,
        entries: Vec<(RepositoryId, GitStatusEntry)>,
        cx: &mut Context<Self>,
    ) {
        let mut by_repository: HashMap<RepositoryId, Vec<GitStatusEntry>> = HashMap::default();
        for (repository_id, entry) in entries {
            by_repository.entry(repository_id).or_default().push(entry);
        }
        for (repository_id, entries) in by_repository {
            let Some(repository) = self.repository_for_id(repository_id, cx) else {
                continue;
            };
    ...
    pub fn stash_selected(&mut self, _: &StashFile, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.prompt_for_stash_message(StashKind::Selected(repository_id, paths), window, cx);
    }
    ...
    fn perform_stash(&mut self, kind: StashKind, message: Option<String>, cx: &mut Context<Self>) {
        // `Selected` carries the repository it was validated against; the
        // other kinds always act on the active repository.
        let stash_repository = match &kind {
            StashKind::Selected(repository_id, _) => {
                match self.repository_for_id(*repository_id, cx) {
                    Some(repository) => repository,
                    None => return,
                }
            }
            _ => match self.active_repository.clone() {
                Some(repository) => repository,
                None => return,
            },
        };
        ...
        cx.spawn({
            async move |this, cx| {
                ...
                let stash_task = stash_repository
                    .update(cx, |repo, cx| match kind {
                        ...
                        StashKind::Selected(_, paths) => repo.stash_entries(paths, message, cx),
                    })
                    .await;
                this.update(cx, |this, cx| {
                    stash_task
    ...
    fn toggle_staged_for_selected(
        ...
        if entries.len() > 1 {
            // Stage unless every entry is already staged; pending-op-aware
            // status is resolved per owning repository.
            let stage = entries.iter().any(|(repository_id, entry)| {
            ...
            self.change_file_stage_grouped(stage, entries, cx);
            return;
        }
    ...
    fn stage_selected(&mut self, _: &git::StageFile, _window: &mut Window, cx: &mut Context<Self>) {
        let to_stage = self
            ...
            .into_iter()
            .filter(|(_, entry)| entry.staging != StageStatus::Staged)
            .collect::<Vec<_>>();
        if !to_stage.is_empty() {
            self.change_file_stage_grouped(true, to_stage, cx);
        }
    }
    ...
    fn unstage_selected(
        ...
        let to_unstage = self
            .effective_status_entries()
            .into_iter()
            .filter(|(_, entry)| entry.staging != StageStatus::Unstaged)
            .collect::<Vec<_>>();
        if !to_unstage.is_empty() {
            self.change_file_stage_grouped(false, to_unstage, cx);
        }
    }
    ...
    fn toggle_section(
        ...
        if !self.collapsed_sections_by_repository.insert(key) {
            self.collapsed_sections_by_repository.remove(&key);
        }
        self.update_visible_entries(window, cx);
    }
    ...
    fn update_visible_entries(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.collapsed_sections_by_repository
            ...
            for (section, entries) in section_entries {
                ...
                let section_is_visible = repository_contents_are_visible
                    && (!section_has_header
                        || !self
                            .collapsed_sections_by_repository
                            .contains(&(repository_id, section)));
        ...
        if self.selected_entry_id.is_none() {
            if let Some(selected_index) = self.selected_entry {
                self.selected_entry_id = self.entry_identity(selected_index);
            }
        }
    ...
    fn toggle_section_collapsed(
        ...
        // Test-only helper: toggles the section for the active repository,
        // mirroring what `toggle_section` does through the UI.
        let Some(repository_id) = self.active_repository_id else {
            return;
        };
        self.toggle_section(repository_id, section, window, cx);
    }
    ...
    fn deploy_entry_context_menu(
        ...
        if matches!(self.entries.get(ix), Some(GitListEntry::Directory(_))) {
            self.selected_entry = Some(ix);
            self.selected_entry_id = self.entry_identity(ix);
            self.selection_anchor = self.selected_entry_id.clone();
        ...
        let single_stage_title = if stage_intent.resolve_with(|| match repo {
        ...
        let (stage_title, restore_title) = if bulk_entries.len() > 1 {
            ...
            let stage_title = if bulk_entries
                .iter()
                .all(|(_, entry)| entry.status.staging().is_fully_staged())
            {
                format!("Unstage {count} Files")
            } else {
                format!("Stage {count} Files")
            };
            let restore_title = if bulk_entries
                .iter()
                .all(|(_, entry)| entry.status.is_created())
            {
        ...
        h_flex()
            ...
            .on_click({
                cx.listener(move |this, event: &ClickEvent, window, cx| {
                    ...
                    this.select_entry_from_pointer(ix, shift, toggle);
                    cx.notify();
                    if !shift && !toggle {
...
mod tests {
    ...
    async fn test_group_by_staging_section_membership_and_order(cx: &mut TestAppContext) {
        ...
        panel.update_in(&mut cx, |panel, _window, _cx| {
            ...
            let staged_id = panel
                .entry_identity(staged_index)
                .expect("staged identity should exist");
            let unstaged_id = panel
                .entry_identity(unstaged_index)
    ...
    mod stash_selection_tests {
        ...
        fn rejects_empty_or_invalid_paths_without_stashing() {
            let error =
                validate_selected_stash_paths(RepositoryId(1), [(RepositoryId(1), path(""))])
                    .unwrap_err();
            assert_eq!(
                error.to_string(),
                "cannot stash an empty or invalid selected path"
