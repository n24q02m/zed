...
impl LogSource {
    ...
    fn get_args(&self) -> Vec<Cow<'_, str>> {
        ...
        // ambiguous. `Path` states its own separator before the path it passes; `Selection`
        // embeds its path in `-L` and has no revision argument.
        if !matches!(self, LogSource::Path(_) | LogSource::Selection { .. }) {
